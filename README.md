...
Для запуска без Docker -->  нужно в <u>файле database.go</u> изменить порт с `db_AuthServ` на `localhost`, но база данных при этом, все равно должна быть поднята в Docker. Или можете настроить подключение к БД вручную, все в том же файле.
...
# Как думал делать? 
Делать решил через фреймворк GIN. Было понимание, что такое токены, зачем нужны и ранее, уже делал их.
...
- в БД с сессиями есть поле GUID, оно должно быть уникальное, дубликат не получиться сделать. Если у RefreshToken закончится время и с таким попытаються зайти, то запись с ним удалится. При Refresh операции, старая запись удаляется.   
